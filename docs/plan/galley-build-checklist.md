...
- [x] npm publish prep DONE and verified. Scoped `@travis-gilbert/markdown-theory` (renderer) + `@travis-gilbert/markdown-spine` (parser), v0.1.0, `publishConfig.access=public`, repository/homepage/author, MIT. tsup dist build ships compiled ESM (`.js` + `.d.ts`) with JSX compiled to the automatic runtime; `prepack` builds it; dev `exports` stay on src so the workspace/CI are unaffected. Verified by an external-consumer smoke test (install both tarballs, render with zero JSX config: 6/6 pass), which caught and fixed a real `React is not defined` bug the earlier raw-TSX ship would have shipped. On GitHub (github.com/Travis-Gilbert/markdown-theory) with green CI. Blocked only on `npm login`; then `pnpm -r publish`.
